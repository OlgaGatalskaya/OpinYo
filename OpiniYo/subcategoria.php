...
$sql = "SELECT * FROM subcategoria
        JOIN categoria ON CategoriaSubcategoria = IDCategoria
...
// Устанавливаем видимость пагинации
$showPagination = $pages > 1;

// Управление активностью кнопок "Следующая" и "Предыдущая"
$Previous = max(1, $page - 1); // Не меньше 1
$Next = min($page + 1, $pages); // Не больше общего количества страниц

// Определяем, активны ли кнопки "Следующая" и "Предыдущая"
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="categoria.php?IDCategoria=<?php print $resultado[0]['IDCategoria']?>">Categoria</a></li>
                <li class="breadcrumb-item active" aria-current="page"><?php print $resultado[0]['NombreSubcategoria'] ?></li>
            </ol>
        </nav>
    </div>
</section>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="columna-opiniones col-12 col-lg-8">
                <div class="row mb-3">
                    <h2><?php print $resultado[0]['NombreSubcategoria'] ?></h2>
                </div>
                <div class="row">
                    <hr>
                </div>
                ...
                <div class="row mb-4">
                    <div class="fs-5 fw-bold">Total opiniones: <?php print $numTotalOpiniones = (calcularTotalOpinienesSubcategoria($IDSubcategoria)); ?></div>
                </div>
                <div class="cards" id="cards">
                    <?php $todas_opiniones = todas_opinionesSubcategoria($IDSubcategoria, $start, $limit);
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination justify-content-center">
                                <li class="page-item <?= $previousDisabled ?>">
                                    <a class="page-link" href="subcategoria.php?IDSubcategoria=<?= $resultado[0]['IDSubtegoria'] ?>&page=<?= $Previous ?>" tabindex="-1">Anterior</a>
                                </li>
                                <?php for($i = 1; $i <= $pages; $i++) { ?>
                                    <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                        <a class="page-link" href="subcategoria.php?IDSubategoria=<?= $resultado[0]['IDSubategoria'] ?>&page=<?= $i ?>"><?= $i ?></a>
                                    </li>
                                <?php } ?>
                                <li class="page-item <?= $nextDisabled ?>">
                                    <a class="page-link" href="subcategoria.php?IDSubcategoria=<?= $resultado[0]['IDSubcategoria'] ?>&page=<?= $Next ?>">Siguiente</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
